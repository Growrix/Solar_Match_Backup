...
const GovernmentNewsSection = () => {
  const { articles, error, loading, refresh, lastUpdated } = useNews(6, true);
  ...
  return (
    <section className="bg-gradient-to-br from-onyx-200 to-night-300 py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* No Solar Articles Found */}
        {!loading && articles.length === 0 && !error && (
          <div className="text-center py-12">
            <Sun className="h-16 w-16 text-battleship_gray-600 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-white mb-2">No solar news available</h3>
            <p className="text-battleship_gray-700">We're filtering for solar and renewable energy content. Check back later for updates.</p>
          </div>
        )}
        ...
        {articles.length > 0 && (
          <div className="text-center mt-12">
            <button
              onClick={() => window.dispatchEvent(new CustomEvent('navigate-to-news'))}
