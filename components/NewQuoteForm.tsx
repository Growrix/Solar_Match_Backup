...
import { createSolarQuote } from '@/lib/services/solarQuotes';
...
const NewQuoteForm: React.FC<NewQuoteFormProps> = ({ onComplete }) => {
  ...
  // Check for stored quote type on component mount
  useEffect(() => {
    const storedQuoteType = localStorage.getItem('selected_quote_type');
    
    // If we have a stored quote type, use it and go directly to contact details
    if (storedQuoteType && (storedQuoteType === 'call_visit' || storedQuoteType === 'written')) {
      setQuoteType(storedQuoteType);
      setCurrentPhase('contact-details');
      
      // Clear the stored value to prevent it from affecting future visits
      localStorage.removeItem('selected_quote_type');
      
      console.log('Using stored quote type:', storedQuoteType);
    }
  }, []);
  ...
  const handleSubmitQuoteRequest = async () => {
    ...
    try {
      ...
      await createSolarQuote({
        name: contactDetails.name,
        email: contactDetails.email,
        phone: contactDetails.phone,
        location: 'Sydney', // These would come from the instant quote form in a real implementation
        state: 'NSW',
