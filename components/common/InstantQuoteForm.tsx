...
const InstantQuoteForm: React.FC<InstantQuoteFormProps> = ({ onProceedToDetailedQuote }) => {
  ...
  const handleCalculateQuote = async () => {
    ...
    try {
      ...
      const budget = budgetRanges[formData.budgetRange]
      const totalCost = budget ? (budget.min + budget.max) / 2 : systemSize * 1500
      const federalRebate = Math.round(systemSize * 500)
      const stateRebate = 1000
      const finalPrice = Math.max(totalCost - federalRebate - stateRebate, 0)
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-onyx-700 backdrop-blur-sm border border-onyx-600/30 rounded-xl px-4 py-3 text-white focus:border-giants_orange-500 focus:outline-none transition-colors"
                >
          ...
          <div>
            ...
            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 mb-6">
              ...
              <ul className="text-yellow-200 text-sm space-y-1">
                {quoteResult.disclaimers.map((disclaimer: string, index: number) => (
